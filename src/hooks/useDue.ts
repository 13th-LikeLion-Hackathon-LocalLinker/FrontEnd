...
async function fetchClosingSoonAll(pageSize: number, maxPages: number) {
  const variants: Variant[] = [
    { sizeKey: 'size',     pageKey: 'page',       start: 0, zeroBased: true,  label: 'size/page (0-based)' },
    { sizeKey: 'pageSize', pageKey: 'page',       start: 0, zeroBased: true,  label: 'pageSize/page (0-based)' },
    { sizeKey: 'limit',    pageKey: 'page',       start: 0, zeroBased: true,  label: 'limit/page (0-based)' },
    { sizeKey: 'size',     pageKey: 'pageNumber', start: 1, zeroBased: false, label: 'size/pageNumber (1-based)' },
  ];
  ...
  for (const v of variants) {
    ...
    for (let i = 0, page = v.start; i < maxPages; i++, page++) {
      ...
      console.log('[closing-soon][request]', { variant: v.label, i, page, effectiveSize, url });

      let res: any;
      try {
        res = await fetchJSON(url);
      } catch (e) {
        console.log('[closing-soon][request:error]', { variant: v.label, i, page, e });
        break;
      }

      const items = unpackArray(res);
      const ids = items.map(x => x.id);
      console.log('[closing-soon][response]', { variant: v.label, i, page, count: items.length, ids });
      ...
      if (i > 0 && sig === prevIdsSig) {
        console.log('[closing-soon][end:repeated-page]', { variant: v.label, i, page });
        break;
      }
      prevIdsSig = sig;
      ...
      console.log('[closing-soon][dedupe]', { variant: v.label, i, page, added: after - before, uniqueSoFar: after });

      // 힌트 사용(있다면)
      const hasNext      = typeof res?.hasNext === 'boolean' ? (res.hasNext as boolean) : undefined;
      const totalPages   = typeof res?.totalPages === 'number' ? (res.totalPages as number) : undefined;

      // 서버가 cap을 걸어 첫 응답이 요청보다 작으면, 그 크기로 보정
      if (i === 0 && items.length > 0 && items.length < effectiveSize) {
        effectiveSize = items.length;
        console.log('[closing-soon][cap-detected]', { variant: v.label, newEffectiveSize: effectiveSize });
      }

      // 종료 조건
      if (hasNext === false) {
        console.log('[closing-soon][end:hasNext=false]', { variant: v.label, i, page });
        break;
      }
      if (typeof totalPages === 'number') {
        const reached = v.zeroBased ? (page + 1) >= totalPages : page >= totalPages;
        if (reached) {
          console.log('[closing-soon][end:totalPages]', { variant: v.label, i, page, totalPages });
          break;
        }
      }
      if (hasNext === undefined && totalPages === undefined && items.length < effectiveSize) {
        console.log('[closing-soon][end:short-page-no-hints]', { variant: v.label, i, page, got: items.length, expect: effectiveSize });
        break;
      }
    }

    const result = Array.from(byId.values());
    console.log('[closing-soon][variant:summary]', v.label, { uniqueCount: result.length });
  ...
  if (best.length > 0) {
    console.log('[closing-soon][variant:chosen]', bestLabel, { uniqueCount: best.length });
    return best;
  }
  ...
  try {
    ...
    console.log('[closing-soon][fallback:single-call:result]', { count: once.length, ids: once.map(x => x.id) });
    return once;
  } catch (e) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log('[closing-soon][hook:done]', { received: raw.length, mapped: mapped.length });
      } catch (e: any) {
        if (!cancelled) {
