...
export function useCategoryResults(params: Params) {
  ...
  React.useEffect(() => {
    ...
    const url = `/api/postings/category?` + buildQS({
      category: params.cat,
    ...
    fetchJSONOrMock<BackendNotice[]>(url, { signal: ac.signal }, MOCK_BACKEND_LATEST)
      .then(({ data, usedMock }) => {
      ...
      .catch((e) => {
        console.warn('[useCategoryResults] fallback to MOCK due to error:', (e as Error)?.message);
        const raw = MOCK_BACKEND_LATEST.filter((n) => n.category === params.cat);
        setList(mapBackendList(raw));
        setError(null);
        setUsedMock(true);
      })
